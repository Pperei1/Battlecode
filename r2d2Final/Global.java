...
public class Global{
    ...
    public static boolean findBuildDirection(RobotType robot){
        ...
        while(!rc.canBuildRobot(robot,buildDirection)){
            buildDirection = buildDirection.rotateLeftDegrees(3f);
            add = add+3f;
            if(add == 360f){
                return false;
            }
    ...
    public static boolean findBuildDirectionSpecial(RobotType robot, float offset){
        float add = 0f;
        while(!rc.canBuildRobot(robot,buildDirection)){
            buildDirection = buildDirection.rotateLeftDegrees(offset);
            add = add+offset;
            if(add >= 360.0f){
                return false;
            }
    ...
    public static boolean bug2MoveAlex(MapLocation begin, MapLocation end, float arrivalDistance) throws GameActionException {
            ...
            if (max_i > -1) {
                ...
                if(rc.canMove(lastDirectionUsed)){
