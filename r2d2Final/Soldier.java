...
public class Soldier extends Global{
    ...
    public static void loop() throws GameActionException{
        ...
        while(true){
            try{
                updateRobot();
                if(nearEnemyRobot.length != 0 && (rc.getRoundNum()>300 || oneEnemyNotArchon())){
                    if(targetEnemy != null){
                        if(rc.canSenseRobot(targetEnemy.getID()) && ((targetEnemy.getType() == RobotType.SOLDIER) || (targetEnemy.getType() == RobotType.TANK))){
                            ...
                            mc = new MovementController(targetEnemy.getLocation(),rc,1.0f);
                        }
                    }
                    else{
                        targetEnemy = newTarget();
                    ...
                    target = findTarget();
                    mc = new MovementController(target,rc,1.0f);
                    mc.move(5.0f);
                }
                else{
                    if(mc.move(5.0f)){
    ...
    public static MapLocation findTarget() throws GameActionException{
        ...
        if(targetB == null){
            turn = rc.readBroadcast(906);
            if(rc.getRoundNum() - turn < 30 && turn > 1){
                targetB = new MapLocation(rc.readBroadcast(907),rc.readBroadcast(908));
            }
            else{
                targetB = null;
            }
    ...
    public static void lineOfSight(MapLocation m,Direction d, boolean checkingLineOfSight) throws GameActionException{
        direct = whatWillIShoot(m,d,1.1f);
        if(!checkingLineOfSight){
            triad[0] = whatWillIShoot(m,d.rotateLeftDegrees(20f),1.1f);
            triad[1] = whatWillIShoot(m,d.rotateRightDegrees(20f),1.1f);
            pentad[0] = whatWillIShoot(m,d.rotateLeftDegrees(15f),1.1f);
            pentad[1] = whatWillIShoot(m,d.rotateLeftDegrees(30f),1.1f);
            pentad[2] = whatWillIShoot(m,d.rotateRightDegrees(15f),1.1f);
            pentad[3] = whatWillIShoot(m,d.rotateRightDegrees(30f),1.1f);
        }
    }
    ...
    private static void shoot(RobotInfo r) throws GameActionException{
        ...
        if(rc.canFirePentadShot() && p && (rc.getTreeCount()>4 || rc.getTeamBullets() > 70)){
            rc.firePentadShot(d);
        }
    ...
    private static boolean moveToDodge(RobotInfo r) throws GameActionException{
        ...
        if(here.distanceTo(r.getLocation()) < 5.5 && targetEnemy.getType() != RobotType.GARDENER){
            ...
            for(int i=0;i<8;i++){
                ...
                distance = 1.6f;
                if(rc.canMove(currentDir) && isValidMove(r.getLocation().distanceTo(here.add(currentDir,0.8f)),r) && hasLineOfSight(here.add(currentDir,0.8f),r)){
    ...
    private static boolean isValidMove(float distanceToEnemy, RobotInfo r){
        if(r.getType() == RobotType.LUMBERJACK){
            ...
            return(distanceToEnemy > 6.1f && distanceToEnemy < 7f);
        }
        else{
